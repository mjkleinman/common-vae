...
def save_model(model, directory, metadata=None, filename=MODEL_FILENAME):
    ...
    if metadata is None:
        ...
        metadata = dict(img_size=model.img_size, latent_dim=model.latent_dim,
                        model_type_enc=model.model_type_enc, model_type_dec=model.model_type_dec)
...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME):
    ...
    model_type_enc = metadata["model_type_enc"]
    model_type_dec = metadata["model_type_dec"]
    path_to_model = os.path.join(directory, filename)
    model = _get_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq, action_dim, device, path_to_model)
    return model
...
def _get_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq, action_dim, device, path_to_model):
    """ Load a single model.
    ...
    """
    model = init_specific_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq, action_dim).to(device)

...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME):
    ...
    path_to_model = os.path.join(directory, filename)
    model = _get_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq, device, path_to_model)
    return model
...
def _get_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq, device, path_to_model):
    """ Load a single model.
    ...
    """
    model = init_specific_model(model_type_enc, model_type_dec, img_size, latent_dim, latent_dim_unq).to(device)
